...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sqlcheck = @"SELECT * FROM `employer_account` WHERE `username`='$_POST[username]'";
        $resultusername = mysqli_query($con, $sqlcheck);
        $value = mysqli_num_rows($resultusername);
        if ($value >= 1) {
                $sqlaccount = @"UPDATE `employer_account` SET `password`='$_POST[password]' WHERE `username`='$_POST[username]'";
